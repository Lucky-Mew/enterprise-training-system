  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      ...
      <el-upload
          ...
          action="http://localhost:9999/staffUser/import"
    ...
    <el-dialog v-model="data.formVisible" title="企业员工信息" width="500" destroy-on-close="">
      <el-form status-icon ref="formRef" :model="data.form" :rules="data.rules" label-width="80px" style="padding: 20px 30px 10px 0">
        ...
        <el-form-item prop="role" label="身份">
          <el-select v-model="data.form.role" size="large" style="width: 240px" placeholder="请选择身份">
            <el-option label="企业导师" value="2" />
            <el-option label="企业员工" value="3" />
          </el-select>
        </el-form-item>
...
const add = () => {
  ...
  formRef.value.validate((valid) => {
    if (valid) { //验证通过的时候
      request.post('staffUser/add',data.form).then(res => {
        if (res.code === '200') {
...
const update = () => {
  formRef.value.validate((valid) => {
    if (valid) { //验证通过的时候
      request.put('staffUser/update',data.form).then(res => {
        if (res.code === '200') {
          data.formVisible = false
...
const handleDelete = (id) => {
  ElMessageBox.confirm(
      ...
      '提示',
      {
        ...
        request.delete('staffUser/delete/' + id).then(res => {
          if (res.code === '200') {
...
const deleteBatch = () => {
  if (data.rows.length === 0){
    ...
    ElMessageBox.confirm(
      ...
      request.delete('staffUser/deleteBatch',{data: data.rows}).then(res => {
        if (res.code === '200') {
          ElMessage.success('删除成功')
          load()
        } else {
...
const exportData = () => {
  ...
  let url = `http://localhost:9999/staffUser/export?username=${data.username === null ? '' : data.username}`
